...
class TextPreprocessor:
    """
    Preprocessing du texte pour analyses NLP
    
    VERSION CORRIGÉE:
    - Stopwords FR + EN
    - Stopwords custom étendus
    - Nettoyage HTML complet
    - Lemmatisation par défaut
    """
    ...
    def __init__(self, language: str = 'french'):
        ...
        Args:
            language: Langue principale (mais charge FR+EN)
        """
        ...
        custom_stops = {
            ...
            'cv', 'candidature', 'ref', 'code',
            'type', 'lieu', 'détails', 'avantages', 'extraits',
            'complète', 'nbsp',
            
            # Méta-mots offres emploi (EN)
            'job', 'position', 'role', 'offer', 'employment',
            'responsibilities', 'profile', 'work', 'company', 'team',
            'skills', 'qualifications', 'resume', 'application',
            'details', 'benefits', 'summary', 'overview',
            
            # Pronoms possessifs (FR)
            'votre', 'vos', 'notre', 'nos',
            ...
            # Verbes fréquents (FR)
            'est', 'sont', 'être', 'avoir', 'êtes', 'avez', 'avons',
            'fait', 'faites', 'faisons',
            
            # Verbes fréquents (EN)
            'is', 'are', 'be', 'have', 'has', 'been', 'do', 'does'
        }
    ...
    def clean_text(self, text: str) -> str:
        ...
        # Garder uniquement lettres et espaces (avec accents français)
    ...
    def tokenize(self, text: str) -> List[str]:
        ...
        tokens = [
            t for t in tokens 
            if len(t) >= 3  # Au moins 3 caractères
    ...
    def preprocess(self, text: str, lemmatize: bool = True) -> List[str]:
        ...
        # Lemmatize (activé par défaut maintenant)
        if lemmatize:
            tokens = [self.lemmatizer.lemmatize(t) for t in tokens]
        
        return tokens
    
    def preprocess_corpus(self, texts: List[str], lemmatize: bool = True) -> List[List[str]]:
        """
        Preprocess un corpus entier
        
        Args:
            texts: Liste de textes
            lemmatize: Appliquer lemmatisation (défaut: True)
...
if __name__ == "__main__":
    ...
    tokens = preprocessor.preprocess(sample_text, lemmatize=True)
    ...
    print("   4. Lemmatisation activée par défaut")
    print("   5. Normalisation casse pour matching compétences")
