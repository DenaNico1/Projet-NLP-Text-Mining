...
# ============================================
def safe_get(offre, field, default=''):
    """Récupère valeur offre de manière sécurisée"""
    try:
        value = offre.get(field, default)
...
def safe_slice(text, max_len=300):
    """Découpe texte de manière sécurisée"""
...
SYSTEM_LOADED = (
    embeddings is not None and 
    rf_model is not None and 
    embeddings_model is not None and
    len(embeddings) == len(df_offres)  # Garantie synchronisation
)
