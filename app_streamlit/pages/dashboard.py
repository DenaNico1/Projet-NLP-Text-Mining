...
with col3:
    nb_profils = df_filtered[df_filtered["status"] == "classified"][
        "profil_assigned"
    ].nunique()
    st.markdown(
        f"""
...
with col_left:
    ...
    df_timeline["date_posted"] = pd.to_datetime(
        df_timeline["date_posted"], errors="coerce"
    )
    df_timeline = df_timeline.dropna(subset=["date_posted"])

    # Filtrer à partir de novembre 2025
    df_timeline = df_timeline[df_timeline["date_posted"] >= "2024-11-01"]

    df_timeline["month"] = df_timeline["date_posted"].dt.to_period("M")
    timeline_data = df_timeline.groupby("month").size().reset_index(name="count")

    # Convertir en format datetime pour un meilleur affichage
    timeline_data["month_date"] = timeline_data["month"].dt.to_timestamp()
    timeline_data["month_label"] = timeline_data["month_date"].dt.strftime("%b %Y")

    fig_timeline = px.line(
        timeline_data, x="month_date", y="count", title="", markers=True
    )
    ...
    fig_timeline.update_layout(
        ...
        hovermode="x unified",
        xaxis=dict(tickformat="%b %Y", dtick="M1"),  # Un tick par mois
    )
...
with col_right:
    ...
    fig_profils = go.Figure(
        go.Bar(
            x=top_profils.values,
            y=top_profils.index,
            orientation="h",
            marker=dict(
                color=top_profils.values, colorscale="Viridis", showscale=False
            ),
...
with col_left2:
    ...
    top_comp = pd.DataFrame(
        comp_counts.most_common(15), columns=["competence", "count"]
    )
...
with col_right2:
    ...
    fig_sources.update_layout(template="plotly_dark", height=500, showlegend=True)
...
with col_map:
    ...
    fig_regions.update_traces(marker_color=COLORS["info"], textposition="outside")
...
with col_stats:
    ...
    st.metric(
        "Tokens par Offre", f"{tokens_moyen:.0f}", help="Longueur moyenne descriptions"
    )
    ...
    st.metric("Région Dominante", region_top, help="Région avec le plus d'offres")
...
with col2:
    with st.expander("📍 Concentration Géographique"):
        idf_pct = (
            (df_filtered["region"] == "Île-de-France").sum() / len(df_filtered) * 100
        )
        st.markdown(
            f"""
