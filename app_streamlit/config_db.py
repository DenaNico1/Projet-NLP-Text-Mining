...
CORRECTION : Fonction get_db_connection() sans cache
...
# ============================================
# CONNEXION (CORRIGÉE - SANS CACHE)
# ============================================

def get_db_connection():
    """
    Crée une NOUVELLE connexion PostgreSQL à chaque appel
    
    CORRECTION : Pas de cache @st.cache_resource
    → Évite problème "connection already closed"
    ...
    try:
        ...
        # Vérifier connexion vivante
        if conn.closed:
            raise Exception("Connexion créée mais fermée immédiatement")
...
def load_offres_complete():
    ...
    try:
        ...
        conn.close()  # CORRECTION : Fermer connexion après usage
...
def load_offres_with_nlp():
    ...
    # Utiliser directement la vue v_offres_nlp_complete
    query = "SELECT * FROM v_offres_nlp_complete"
    
    try:
        df = pd.read_sql(query, conn)
        conn.close()  # CORRECTION : Fermer connexion
        return df
    except Exception as e:
        if conn:
            conn.close()
